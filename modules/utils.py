...
def select_device(device='', batch_size=None):
    ...
    # s = f'YOLOv3 🚀 {git_describe() or date_modified()} torch {torch.__version__} '  #commented out by jy# string
    ...
    if cuda:
        ...
        # space = ' ' * len(s)   #commented out by JY
        for i, d in enumerate(devices):
            p = torch.cuda.get_device_properties(i)
            # s += f"{'' if i == 0 else space}CUDA:{d} ({p.name}, {p.total_memory / 1024 ** 2}MB)\n"  # commented out by JY, bytes to MB
    # else:
        # s += 'CPU\n'  #commented out by JY
